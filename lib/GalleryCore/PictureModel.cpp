...
void PictureModel::addPictures(const QVector<Picture>& pictures)
{
	int rows = rowCount();
	beginInsertRows(QModelIndex(), rows, rows+pictures.size());
	mDb.pictureDao.addPicturesInAlbum(mAlbumId, pictures);
	for(int i=0; i < pictures.size(); i++)
	{
		unique_ptr<Picture>newPicture(new Picture(pictures.at(i)));
		mPictures->push_back(move(newPicture));
	}
	endInsertRows();
}
